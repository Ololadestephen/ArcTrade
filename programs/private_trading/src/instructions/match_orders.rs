...
use crate::constants::{IX_MATCH_ORDERS, MAX_ENCRYPTED_ORDER_BLOB};
...
pub struct MatchOrdersArgs {
    pub encrypted_match_blob: Vec<u8>,
}
...
pub fn match_orders(ctx: Context<MatchOrders>, args: MatchOrdersArgs) -> Result<()> {
    ...
    let encrypted_match_blob = bounded_blob(args.encrypted_match_blob, MAX_ENCRYPTED_ORDER_BLOB)?;
    ...
    let computation_id = compute_deterministic_id(
        ...
        user_position.nonce,
        IX_MATCH_ORDERS,
        &encrypted_match_blob,
    );
    user_position.pending_computation_id = computation_id;
    user_position.pending_instruction = IX_MATCH_ORDERS;

    emit!(OrderMatched {
        match_hash: computation_id,
    });
    Ok(())
}
