...
use crate::constants::{IX_CHECK_LIQUIDATION, MAX_ENCRYPTED_POSITION_BLOB};
...
pub struct CheckLiquidationArgs {
    pub encrypted_liquidation_blob: Vec<u8>,
}

pub fn check_liquidation(ctx: Context<CheckLiquidation>, args: CheckLiquidationArgs) -> Result<()> {
    require!(!ctx.accounts.market.paused, PrivateTradingError::MarketPaused);

    let encrypted_liquidation_blob =
        bounded_blob(args.encrypted_liquidation_blob, MAX_ENCRYPTED_POSITION_BLOB)?;
    ...
    let computation_id = compute_deterministic_id(
        ...
        &encrypted_liquidation_blob,
    );
    user_position.pending_computation_id = computation_id;
    user_position.pending_instruction = IX_CHECK_LIQUIDATION;
