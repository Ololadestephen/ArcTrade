...
use crate::constants::{IX_SETTLE_TRADE, MAX_ENCRYPTED_POSITION_BLOB};
...
pub struct SettleTradeArgs {
    pub encrypted_settlement_blob: Vec<u8>,
}
...
pub fn settle_trade(ctx: Context<SettleTrade>, args: SettleTradeArgs) -> Result<()> {
    ...
    let encrypted_settlement_blob =
        bounded_blob(args.encrypted_settlement_blob, MAX_ENCRYPTED_POSITION_BLOB)?;
    ...
    let computation_id = compute_deterministic_id(
        ...
        user_position.nonce,
        IX_SETTLE_TRADE,
        &encrypted_settlement_blob,
    );
    user_position.pending_computation_id = computation_id;
    user_position.pending_instruction = IX_SETTLE_TRADE;
